    ...
    if (strcasecmp(line, "help") == 0 || strcmp(line, "?") == 0) {
        ...
        if (s_unicast_engine) {
            ...
            for (int i = 0; i < cnt; i++) {
                ...
                print_console(" [%d] %-8s | MAC: %02X:%02X:%02X:%02X:%02X:%02X | CH: %u | %s | Air: %4lu us (avg %4lu us) | Sent: %lu | ACKs: %lu | Fails: %lu\n",
                              i, p->name,
                              p->mac[0], p->mac[1], p->mac[2], p->mac[3], p->mac[4], p->mac[5],
                              p->channel_id, st_str,
                              (unsigned long)p->last_tx_duration_us,
                              (unsigned long)p->ema_tx_duration_us,
